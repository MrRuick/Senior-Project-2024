...
volatile int FOV = 60;
volatile int spacing = 5;
volatile int sfactor = 5;
...
void PUT_YOUR_LAB_CODE_HERE(){

	//disregard, dimensions for array range
	int xlength = sfactor*2*(2*sin(FOV*(3.14159265358979323846/180)) + 2*spacing);
	int arrx[xlength];
	int ylength = sfactor*(2*cos(FOV*(3.14159265358979323846/180)));
	int arry[ylength];
...
void PINCHECK(){
	if(HAL_GPIO_ReadPin (GPIOB, GPIO_PIN_15)){
		//record timer value
		timer_val1 = __HAL_TIM_GET_COUNTER(&htim2) - timer_val1;
		//test serial message
		sprintf(message, "Welcome to lab!");
		HAL_UART_Transmit(&huart1, message, sizeof(message), 100);
		//toggle on-board led for confirmation
